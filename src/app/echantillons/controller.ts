...
export async function addEchantillon(values: any) {
  ...
  const tableColumns = Object.keys(dataBase.echantillons.columns).filter(
    (e) => values[e as keyof object]
  )
  ...
  if (values["selectionaliquote"]) {
        ...
        await asyncForEach(ids[0].ids, async (id: string) => {
          await readId(dataBase.echantillons.name, +id).then((echantillon: any) => {
            ...
            for (let loop = 0; loop < nb; loop++) {
              ...
              queries.push(
                `INSERT INTO ${dataBase.echantillons.name} (${insertInto.join()}) VALUES (${createPgValues(dataBase.echantillons.name, echantillon)})`
              )
            }
          })
        })
    ...
    await readId(dataBase.excels.name, +values["excel" as keyof object])
      .then((excelFile: any) => {
        // Create list of excel columns
        const excelCols = excelFile[0]["datas" as keyof object]["columns"]
        // lopp excel lines
        Object.values(excelFile[0]["datas" as keyof object]["datas"]).forEach(
          (tmp: any, index: number) => {
            // clone line
            ...
            if (tempVal["identification"])
              tempVal["identification"] =
                values["identification" as keyof object].slice(0, 12) +
                String(+tmp[excelCols["echantillon" as keyof object]] || index + start).padStart(
                  4,
                  "0"
                )
            ...
            // Go
            queries.push(
              `INSERT INTO ${dataBase.echantillons.name} (${insertInto.map((e) => `"${e}"`).join()}) VALUES (${createPgValues(dataBase.echantillons.name, tempVal, insertInto)})`
            )
          }
        )
      })
    ...
    codesIdentification.forEach((identification, i) => {
      ...
      queries.push(
        `INSERT INTO ${dataBase.echantillons.name} (${insertInto.join()}) VALUES (${createPgValues(dataBase.echantillons.name, values)})`
      )
    })
    // execute all queries
    return new Promise(async function (resolve, reject) {
      return executeSql(queries)
        .then(async () => {
          ...
          const selectionId = await sql.unsafe(
            `INSERT INTO ${dataBase.selections.name} (ids) SELECT ARRAY_AGG(id) FROM ${dataBase.echantillons.name} WHERE identification IN ('${codesIdentification.join("','")}') RETURNING id`
          )
...
export async function updateEchantillon(values: any, id: number) {
  return new Promise(async function (resolve, reject) {
    return await executeSql(
      `UPDATE ${dataBase.echantillons.name} SET ${createPgUpdates(dataBase.echantillons.name, values)} WHERE id = ${id}`
    )
      .then(async () => {
