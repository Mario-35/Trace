...
echantillonsRoutes.get("/" + dataBase.echantillons.singular + "/identification/:id", async (req, res) => {
    await executeSql(`SELECT id, ${getListColumns(dataBase.echantillons.name)} FROM ${dataBase.echantillons.name} WHERE identification ${req.params.id.length < 13 ? 'LIKE' : '='} '${ req.params.id }${req.params.id.length < 13 ? '%' : ''}' ORDER BY identification`)
...
echantillonsRoutes.get("/list/" + dataBase.echantillons.name +"/:id", async (req, res) => {
    await executeSql(`SELECT id, ${getListColumns(dataBase.echantillons.name)} FROM ${dataBase.echantillons.name} WHERE passeport = ${ +req.params.id } ORDER BY creation`)
    .then((echantillons: any) => {
        return res.status(200).json(echantillons);
    }).catch (error => {
        return res.status(404).json({"error": error.detail});
