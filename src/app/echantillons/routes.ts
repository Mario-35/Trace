...
echantillonsRoutes.get(
  "/" + dataBase.echantillons.singular + "/identification/:id",
  async (req, res) => {
    await executeSql(
      `SELECT id, ${getListColumns(dataBase.echantillons.name)} FROM ${dataBase.echantillons.name} WHERE identification ${req.params.id.length < 13 ? "LIKE" : "="} '${req.params.id}${req.params.id.length < 13 ? "%" : ""}' ORDER BY identification`
    )
...
echantillonsRoutes.get("/" + dataBase.echantillons.name + "/serie/:id", async (req, res) => {
  await executeSql(
    `SELECT id FROM ${dataBase.echantillons.name} WHERE identification LIKE CONCAT((SELECT SUBSTRING ( identification FROM 1 FOR 12 ) FROM ${dataBase.echantillons.name} WHERE id =${req.params.id}), '%') GROUP BY id ORDER by id`
  )
    .then((echantillon: any) => {
      return res.status(200).json(echantillon)
    })
...
echantillonsRoutes.get("/" + dataBase.echantillons.name, async (req, res) => {
  return await executeSql(
    `SELECT * FROM ${dataBase.echantillons.name} ORDER BY creation, Identification`
  )
...
echantillonsRoutes.get("/list/" + dataBase.echantillons.name + "/:id", async (req, res) => {
  await executeSql(
    `SELECT id, ${getListColumns(dataBase.echantillons.name)} FROM ${dataBase.echantillons.name} WHERE passeport = ${+req.params.id} ORDER BY creation`
  )
    .then((echantillons: any) => {
      return res.status(200).json(echantillons)
    })
    ...
    .then(async (echantillon: any) => {
      if (echantillon.length === 1) {
        ...
        if (Object.keys(echantillon.cultures).length > 0) {
          echantillon.codes = await executeSqlValues(
            `SELECT CONCAT('"', UPPER(code), '" : "',valeur, '"') FROM rpg WHERE UPPER(code) IN ('${Array.from(new Set(Object.values(echantillon.cultures).map((item) => item))).join("','")}')`
          )
        }
        return res.status(200).json(echantillon)
...
echantillonsRoutes.get(
  "/" + dataBase.echantillons.singular + "/next/identification/:id",
  async (req, res) => {
    return await executeSql(
      `SELECT COALESCE( MAX( SUBSTRING ( identification FROM 13 FOR 4 ):: int ), 0) as numero FROM ${dataBase.echantillons.name} WHERE identification LIKE '${String(req.params.id.slice(0, 12))}%' LIMIT 1`
    )
      .then((max: any) => {
...
echantillonsRoutes.get(
  "/" + dataBase.echantillons.singular + "/after/identification/:id",
  async (req, res) => {
    return await executeSql(
      `SELECT MAX( SUBSTRING ( identification FROM 13 FOR 4 ):: int ) FROM ${dataBase.echantillons.name} WHERE identification like CONCAT( ( SELECT SUBSTRING ( identification FROM 1 FOR 12 ) FROM ${dataBase.echantillons.name} WHERE id = ${+String(req.params.id)} ), '%' )`
    )
      .then((max: any) => {
        return res.status(200).json({ max: Number(max[0].max) + 1 })
      })
