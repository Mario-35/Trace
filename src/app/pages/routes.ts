...
import { _CONFIG, EConstant, setConfig } from "../../constant"
...
pagesRoutes.get("/" + dataBase.echantillons.name + "/export/:id", async (req, res) => {
  await executeSql(`SELECT ids FROM selections WHERE id=${+req.params.id}`).then(
    async (ids: any) => {
      await executeSql(`SELECT 
            ...
            FROM ${dataBase.echantillons.name} WHERE id IN (${ids[0].ids})`).then((datas) => {
...
pagesRoutes.get("/print/:type/:id", async (req, res) => {
  switch (req.params.type) {
    ...
    case dataBase.selections.singular:
      return await readId(dataBase.selections.name, +req.params.id)
        .then(async (selection: any) => {
          return await readIds(dataBase.echantillons.name, selection[0].ids)
            .then(async (all: any) => {
    ...
    case "identification":
      return await executeSql(
        `SELECT * FROM ${dataBase.echantillons.name} WHERE identification LIKE '${req.params.id.slice(0, 12)}%'`
      )
        .then(async (all: any) => {
          const html = new Print("echantillon", all)
          res.set("Content-Type", "text/html").send(html.toString())
