...
// Get passeports list for UI
passeportsRoutes.get("/list/" + dataBase.passeports.name, async (req, res) => {
  await executeSql(
    `SELECT id, ${getListColumns(dataBase.passeports.name)} FROM ${dataBase.passeports.name} ORDER BY id`
  )
    .then((passeport: any) => {
...
passeportsRoutes.get("/" + dataBase.passeports.name, async (req, res) => {
  if (req.query.search)
    ...
    return await readAll(dataBase.passeports.name)
      .then((passeport: any) => {
        return res.status(200).json(passeport)
      })
      .catch((error) => {
...
// Get all samples from passport id
passeportsRoutes.get(
  "/list/" + dataBase.passeports.singular + "/" + dataBase.echantillons.name + "/:id",
  async (req, res) => {
    await executeSql(
...
passeportsRoutes.get(
  "/" + dataBase.passeports.singular + "/:tracabilite/:year",
  async (req, res) => {
    if (isNaN(+req.params.tracabilite)) {
      return await executeSql(
        `SELECT * FROM passeports WHERE UPPER(site) = '${String(req.params.tracabilite).toUpperCase()}' AND annee = '${req.params.year}'`
      )
        .then((passeport: any) => {
      ...
      return await executeSql(
        `SELECT * FROM passeports WHERE annee = '${req.params.year}' AND tracabilite = '${+req.params.tracabilite}'`
      )
        .then((passeport: any) => {
          return passeport.length > 0
            ? res.status(200).json(passeport[0])
