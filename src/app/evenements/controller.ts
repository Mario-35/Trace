  ...
  return new Promise(async function (resolve, reject) {
    if (values["serie"]) {
      return await executeSql(
        `SELECT identification, etat AS saveetat FROM ${dataBase.echantillons.name} WHERE identification LIKE CONCAT((SELECT SUBSTRING ( identification FROM 1 FOR 12 ) FROM ${dataBase.echantillons.name} WHERE id =${values["serie"]}), '%') GROUP BY identification, etat`
      )
