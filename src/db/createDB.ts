...
export async function createDB(adminPass: string): Promise<Record<string, string>> {
  ...
  const create = await admin(adminPass)
    .unsafe(
      "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE pid <> pg_backend_pid() AND datname = 'trace';"
    )
    .then(async (res) => {
      // drop database
  ...
  Object.keys(dataBase).forEach((tableName) => {
    ...
    if (dataBase[tableName].create === true) {
      ...
      Object.keys(dataBase[tableName].columns).forEach((columnName: string) => {
        if (
          String(
            dataBase[tableName as keyof object].columns[columnName as keyof object][
              "create" as keyof object
            ]
          ).trim() !== ""
        )
          pgCols.push(
      ...
      // add to queries
      queries.push(
        `CREATE TABLE ${tableName} (${pgCols.join()})${tableName === "echantillons" ? " PARTITION BY LIST(type)" : ""};`
      )
    }
  })
  // Execute queries
  ...
  await asyncForEach(_TYPES, async (name) => {
    await executeSql(
      `CREATE TABLE IF NOT EXISTS "echantillon_${name.replaceAll(" ", "").toLowerCase()}" PARTITION OF echantillons FOR VALUES IN ('${toTitleCase(name)}');`
    ).then(() => {
