  class JSONViewer  {
	...
	showJSON = function(json, maxLvl, colAt) {
	 ...
	 */
	_walk = function(value, maxLvl, colAt, lvl) {
	 ...
	 * @return {Element}
	 */
	_createItemsCount = function(count) {
...
query.addEventListener("click", async function () {
    try {
        const temp = await getDatas(`${window.location.origin}/${url.value}`);
        jsonViewer.showJSON(temp || {});
    } catch (error) {
